...
/* fxp_t is an integer type wider than fixedpt_t, used internally to store
   intermediate values. */
...
/*** fxp_scale() -- multiply fixed-point number by a ratio

Mulitiplies a fixed-point value (the first argument) by another (the
second argument) then divides by a third (the third argument). Returns
the result. Note that the result is truncated rather than rounded, and
there is no overflow detection nor protection against division by zero.

Note that the intermediate result (after the multiplication but before
the division) need not fit in a fixedpt_t. So long as the final result
is within range, the return will be as expected.
***/
CONSTFUNC fixedpt_t fxp_scale(const fixedpt_t a, const fixedpt_t mul,
 const fixedpt_t div) {
   return (fxp_t)a * (fxp_t)mul / (fxp_t)div;
}
...
/* http://arduino.cc/en/Reference/PROGMEM says: "Some cryptic bugs are
   generated by using ordinary datatypes for program memory calls." Vague,
   and prog_int16_t doesn't seem to be in the headers anymore as of
   24 May 2014. So, we'll just use fixedpt_t here: */
static const fixedpt_t _tbl[] PROGMEM = {
...
***/
CONSTFUNC fixedpt_t fxp_atan2(fixedpt_t y, fixedpt_t x) {
...
/*** fxp_abs() -- get absolute value of fixed-point number

Returns the absolute value of the fixed-point number passed as an argument.
***/
CONSTFUNC fixedpt_t fxp_abs(const fixedpt_t x) { return x >= 0 ? x : -x; }
